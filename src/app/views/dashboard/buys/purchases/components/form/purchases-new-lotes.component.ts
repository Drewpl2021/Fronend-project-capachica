...
@Component({
    selector: 'app-puchases-new-lotes',
    standalone: true,
        ...
        <div class="flex flex-col">
            ...
            <form class="flex flex-col flex-auto p-6 sm:p-8 overflow-y-auto" [formGroup]="purchasesForm">
                <div class="flex items-center justify-end mb-4">
                    <button type="button" class="bg-green-500 text-white px-4 py-2 rounded-md shadow hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1" (click)="addLote()">
                        Añadir Lote
                    </button>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 border">
                        ...
                        <tr *ngFor="let lote of lotes.controls; let i = index" [formGroupName]="i" class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center">
                                <button type="button" (click)="deleteLot(i)" aria-label="Eliminar">
...
export class PurchasesNewLotesComponent implements OnInit {
    purchasesForm: FormGroup;
    constructor(
        private fb: FormBuilder,
        private _matDialog: MatDialogRef<PurchasesNewLotesComponent>,
    ...
    private initializeLots(): void {
        const quantity = 1;
        const lotsActuals = this.data.lotes || [];
        this.lotes.clear();
        lotsActuals.forEach((lote: any) => {
        ...
        for (let i = lotsActuals.length; i < quantity; i++) {
            this.lotes.push(this.fb.group({
    ...
    deleteLot(index: number): void {
        this.lotes.removeAt(index);
    }
    saveForm(): void {
        if (this.purchasesForm.valid) {
            this._matDialog.close(this.purchasesForm.value.lotes);
        }
    }
    cancelForm(): void {
