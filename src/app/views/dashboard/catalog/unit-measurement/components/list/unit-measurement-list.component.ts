        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tr class="bg-primary-600 text-white">
                    ...
                    <th class="w-1/9 table-header text-center border-r">Cod. Sunat</th>
                    <th class="w-1/9 table-header text-center border-r">Si. Impre</th>
                ...
                <tbody class="bg-white">
                    @for (measurement of unitMeasurements; track measurement.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ measurement.sunatCode }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ measurement.symbolPrint }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ measurement.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ measurement.updatedAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{measurement.state===true?'green':'red'}}-500/20 text-{{measurement.state===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ measurement.state === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="flex justify-center space-x-3">
                                    ...
                                    <mat-icon
                                        ...
                                        (click)="goDelete(measurement.id)">delete_sweep
                                    </mat-icon>
                                </div>
                            </td>
                        </tr>
...
export class UnitMeasurementListComponent implements OnInit {
    abcForms: any;
    @Input() unitMeasurements: UnitMeasurement[] = [];
