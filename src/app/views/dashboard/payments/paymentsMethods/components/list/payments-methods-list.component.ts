...
import { PaymentMethods } from '../../models/payments-methods';
...
@Component({
    selector: 'app-payments-methods-list',
    standalone: true,
    imports: [
        FormsModule,
        CommonModule,
        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tr class="bg-primary-600 text-white">
                    ...
                    <th class="w-1/9 table-header text-center border-r">Tipo de Pago</th>
                ...
                <tbody class="bg-white">
                    @for (paymentMethod of paymentMethods; track paymentMethod.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ paymentMethod.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{paymentMethod.state===true?'green':'red'}}-500/20 text-{{paymentMethod.state===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ paymentMethod.state === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
...
export class PaymentsMethodsListComponent implements OnInit {
    abcForms: any;
