        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tbody class="bg-white">
                    @for (document of type_document; track document.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ document.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{document.status===true?'green':'red'}}-500/20 text-{{document.status===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ document.status === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="flex justify-center space-x-3">
                                    <mat-icon
                                        ...
                                        (click)="goEdit(document.id)">edit
                                    </mat-icon>
                                    <mat-icon
                                        matTooltip="Eliminar Tipo de Documento"
                                        matTooltipClass="tooltip-red"
                                        matTooltipPosition="above"
                                        [matTooltipDisabled]
                                        class="text-rose-500 hover:text-rose-600 cursor-pointer"
                                        (click)="goDelete(document.id)">delete_sweep
                                    </mat-icon>
                                </div>
                            </td>
                        </tr>
...
export class TypeDocumentListComponent implements OnInit {
    abcForms: any;
    @Input() type_document: TypeDocument[] = [];
