        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tr class="bg-primary-600 text-white">
                    ...
                    <th class="w-1/9 table-header text-center border-r">Fecha Inicio</th>
                    <th class="w-1/9 table-header text-center border-r">Fecha Vigencia</th>
                ...
                <tbody class="bg-white">
                    @for (prices of prices; track prices.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ prices.startDate | date:'dd/MM/yyyy' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ prices.endDate | date:'dd/MM/yyyy' }}
                            </td>

                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{prices.state===true?'green':'red'}}-500/20 text-{{prices.state===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ prices.state === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="flex justify-center space-x-3">
                                    <mat-icon
                                        ...
                                        (click)="goEdit(prices.id)">
                                        visibility
                                    </mat-icon>
                                    <mat-icon
                                        matTooltip="Eliminar Precio"
                                        matTooltipClass="tooltip-red"
                                        matTooltipPosition="above"
                                        [matTooltipDisabled]
                                        class="text-rose-500 hover:text-rose-600 cursor-pointer"
                                        (click)="goDelete(prices.id)">
                                        delete_sweep
                                    </mat-icon>
                                </div>
                            </td>
