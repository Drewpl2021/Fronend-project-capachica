...
import {Module, SectionDetail, SectionDetailEnd} from "../../models/module";
    ...
    template: `
        <div class="bg-white flex flex-col px-4 py-4  mx-auto rounded shadow-md "  style="width: 10000px">
            <div class="flex justify-between bg-primary-600 text-white p-2 rounded mb-4">
                <h1 class="text-2xl font-bold"> Seccion Detalle De Campos</h1>
            </div>
            ...
            <div>
                ...
                <div class="cards-container" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; padding: 20px;">
                    <div *ngFor="let section of sectionDetails; let idx = index" class="card-container" style="display: flex; justify-content: center;">
                        <div class="card"
                             style="width: 100%; max-width: 350px; height: 450px; transition: transform 0.3s ease-in-out; background-color: #f9fafb; border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); padding: 20px; margin-bottom: 20px; overflow: hidden;">

                            <!-- Imagen de la sección cargada desde internet -->
                            <img *ngIf="section.image" [src]="section.image" alt="{{ section.title }}" style="width: 100%; height: 150px; object-fit: cover; border-radius: 8px; margin-bottom: 15px;">

                            <div class="text-center" style="font-size: 1.25rem; font-weight: bold; margin-bottom: 10px; color: #4CAF50; background-color: #e8f5e9; padding: 10px; border-radius: 5px;">
                                {{ section.title }}
                            </div>
                            ...
                            <div class="text-sm mb-2" style="font-size: 14px; margin-bottom: 8px;">
                                <strong>Subtitulo: </strong>{{ section.subtitle }}
                            </div>
                            <div class="text-sm mb-2" style="font-size: 14px; margin-bottom: 8px;">
                                <strong>Descripcion: </strong>
                                {{ section.description?.length > 40 ? (section.description | slice:0:40) + '...' : section.description }}
                            </div>
...
export class SectionDetailEndViewComponent implements OnInit {
    @Input() title: string = '';
    ...
    constructor(
        ...
        private _sectionsDetailsService: SectionsDetailsEndService,
    ...
    private uploadData() {
        this._sectionsDetailsService.getByIdSS$(this.sectionId).subscribe((data) => {
            // Asignamos la URL de la imagen a cada sección
            this.sectionDetails = data.map((section: any) => {
                section.image = this.fetchImageUrl(section); // Asignamos la imagen
                return section; // Devolvemos la sección con la imagen actualizada
            });
        });
    }
    fetchImageUrl(section: SectionDetailEnd): string {
        // Si hay una URL de la imagen, la retorna, sino, retorna una imagen por defecto
        if (section.image) {
            return section.image;
        } else {
            return 'https://example.com/default-image.jpg'; // Imagen por defecto
        }
    }
    ...
    public goEdit(id: string) {
        this._sectionsDetailsService.getById$(id).subscribe((response) => {
            ...
            const moduleForm = this._matDialog.open(SeccionDetalleEndEditComponent);
