...
import {ContAsientos, CategoryFilter, PaginatedResponse} from '../models/cont-asientos';
...
import {ContAsientosService} from "../../../../../providers/services/cont-asientos/cont-asientos.service";

@Component({
    selector: 'app-cont-asientos-container',
    standalone: true,
    imports: [CategoryListComponent, PaginationControlsComponent, CategoryFilterComponent],
    template: `
        <div class="w-full mx-auto p-6 bg-white rounded overflow-hidden shadow-lg">
            <app-cont-asientos-filter
                (eventFilter)="eventFilter($event)"
                (eventNew)="eventNew($event)">
            </app-cont-asientos-filter>
            <app-cont-asientos-list class="w-full"
                [categories]="categories"
                (eventEdit)="eventEdit($event)"
                (eventDelete)="eventDelete($event)"
            ></app-cont-asientos-list>
            <pagination-controls
...
export class CategoryContainersComponent implements OnInit {
    ...
    constructor(
        ...
        private _contAsientosService:ContAsientosService,
    ) {}
    ...
    private getCategories(data?: any): void {
        this._contAsientosService.getWithQuery$(data).subscribe(
            (response) => {
    ...
    private edit(UnitMeasurement: ContAsientos) {
