        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tr class="bg-primary-600 text-white">
                    ...
                    <th class="w-1/9 table-header text-center border-r">Lugar</th>
                ...
                <tbody class="bg-white">
                    @for (category of categories; track category.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ category.descripcion.length > 50 ? (category.descripcion | slice:0:50) + '...' : category.descripcion }}
                            </td>
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{category.estado===true?'green':'red'}}-500/20 text-{{category.estado===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ category.estado === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
