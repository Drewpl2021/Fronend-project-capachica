    ...
    template: `
        <div class="flex flex-col max-w-240 md:min-w-160 max-h-screen -m-6">
            <div class="flex flex-0 items-center justify-between h-16 pr-3 sm:pr-5 pl-6 sm:pl-8 bg-primary text-on-primary">
                <div class="text-lg font-medium">Editar Cuenta Contable</div>
                <button mat-icon-button (click)="cancelForm()" [tabIndex]="-1">
                    <mat-icon class="text-current" [svgIcon]="'heroicons_outline:x-mark'"
                    ></mat-icon>
                </button>
            </div>
            <form class="flex flex-col flex-auto p-6 sm:p-8 overflow-y-auto" [formGroup]="accountForm">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mt-4 sm:mt-6">
                    <div class="flex space-x-2 items-center mt-4 sm:mt-0">
                        ...
                        <button class="ml-auto sm:ml-0" color="primary" [disabled]="accountForm.invalid" mat-stroked-button (click)="saveForm()">
                            Guardar
                        </button>
                    </div>
                </div>
...
export class AccountingAccountClassEditComponent implements OnInit {
    @Input() title: string = '';
    @Input() unitMeasurement = new accountingAccountClass();
    abcForms: any;
    ...
    constructor(
        private _matDialog: MatDialogRef<AccountingAccountClassEditComponent>
    ) {}
    ngOnInit() {
