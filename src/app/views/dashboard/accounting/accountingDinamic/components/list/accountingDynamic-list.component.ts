...
import { accountingDynamic } from '../../models/accountingDynamic';
...
@Component({
    selector: 'app-accounting-dynamic-list',
    standalone: true,
    imports: [
        FormsModule,
        CommonModule,
        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tr class="bg-primary-600 text-white">
                    ...
                    <th class="w-1/9 table-header text-center border-r">Porcentaje IGV</th>
                ...
                <tbody class="bg-white">
                    @for (dynamic of dynamics; track dynamic.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ dynamic.percentageIgv }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ dynamic.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ dynamic.updatedAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>

                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{dynamic.status===true?'green':'red'}}-500/20 text-{{dynamic.status===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ dynamic.status === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
...
export class AccountingDynamicListComponent implements OnInit {
    abcForms: any;
    @Input() dynamics: accountingDynamic[] = [];
