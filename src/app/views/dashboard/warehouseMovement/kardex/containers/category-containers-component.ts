...
import {KardexService} from "../../../../../providers/services/warehouseMovement/kardex.service";


@Component({
    selector: 'app-kardex-container',
    standalone: true,
...
export class CategoryContainersComponent implements OnInit {
    ...
    constructor(
        private _categoryService: KardexService,
    ...
    private getCategories(data?: any): void {
        const type = 'Ingreso'; // El tipo que deseas enviar
        const params = {
            page: data?.page || 0,
            size: data?.size || 20,
        };

        this._categoryService.getListType$(`${type}`, params).subscribe(
            (response) => {
