...
import { EntitiesCompanyC} from '../../models/entitiesCompanyC';
...
@Component({
    selector: 'app-entities-client-list',
    standalone: true,
    imports: [
        FormsModule,
        CommonModule,
                    ...
                    @for (entity of entities; track entity.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.nameSocialReason }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.documentType?.name }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.documentNumber }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.address }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.email }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.phone }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ entity.entitys?.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="flex justify-center space-x-3">
                                    <mat-icon
                                        ...
                                        (click)="goEdit(entity.entitys?.id)">edit
                                    </mat-icon>
                                    <mat-icon
                                        ...
                                        (click)="goDelete(entity.entitys?.id)">delete_sweep
                                    </mat-icon>
                                </div>
                            </td>
                        </tr>
...
export class EntitiesCompanyCListComponent implements OnInit {
    abcForms: any;
    @Input() entities: EntitiesCompanyC[] = [];
