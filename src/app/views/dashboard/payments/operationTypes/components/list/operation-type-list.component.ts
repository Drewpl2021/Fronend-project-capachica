        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tbody class="bg-white">
                    @for (operationType of operationTypes; track operationType.id; let idx = $index) {
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                {{ operationType.createdAt | date:'dd/MM/yyyy HH:mm:ss' }}
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
                                <div class="w-max">
                                    <div
                                        class="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-{{operationType.state===true?'green':'red'}}-500/20 text-{{operationType.state===true?'green':'red'}}-600 py-1 px-2 text-xs rounded-md"
                                        style="opacity: 1">
                                        <span class="">{{ operationType.state === true ? 'ACTIVO' : 'INACTIVO' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="w-1/9 p-2 text-center border-b text-sm">
